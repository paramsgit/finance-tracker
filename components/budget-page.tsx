...
export function BudgetPage() {
  const { getMonthlyBudget, setCategoryBudget, distributeBudgetEqually, redistributeBudget } = useBudget()
  ...
  const totalBudgetAmount = Object.values(monthlyBudget).reduce((sum, amount) => sum + amount, 0)
  ...
  const categoriesWithBudget = categories.filter((cat) => (monthlyBudget[cat] || 0) > 0)
  const categoriesWithZeroBudget = categories.filter((cat) => (monthlyBudget[cat] || 0) === 0)

  // Redistribute budget when categories change
  useEffect(() => {
    const budgetCategories = Object.keys(monthlyBudget)
    const missingCategories = categories.filter((cat) => !budgetCategories.includes(cat))
    const extraCategories = budgetCategories.filter((cat) => !categories.includes(cat))
  ...
  const renderCategoryBudget = (category: string) => {
    ...
    const categoryPercentage = budgetAmount > 0 ? (spentAmount / budgetAmount) * 100 : 0
    const isOverCategoryBudget = spentAmount > budgetAmount && budgetAmount > 0

    return (
      <div key={category} className="space-y-2 p-4 border rounded-lg dark:border-gray-600">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-medium text-gray-900 dark:text-white">{category}</span>
            {isOverCategoryBudget && <AlertTriangle className="h-4 w-4 text-red-600 dark:text-red-400" />}
          </div>
          <div className="flex items-center gap-2">
            {editingCategory === category ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">${budgetAmount.toFixed(2)}</span>
        ...
        {budgetAmount > 0 && (
          <>
            <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
              ...
              <span
                className={
                  categoryRemaining >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                }
              >
                {categoryRemaining >= 0 ? "Remaining" : "Over"}: ${Math.abs(categoryRemaining).toFixed(2)}
              </span>
            </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Budget</h1>
          <p className="text-gray-600 dark:text-gray-400">Set and track your budget for {selectedMonthLabel}</p>
        </div>
      ...
      <Card className="dark:bg-gray-800 dark:border-gray-700">
        <CardHeader>
          <CardTitle className="text-gray-900 dark:text-white">Set Total Monthly Budget</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSetTotalBudget} className="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <Button type="submit" disabled={!totalBudget || categories.length === 0}>
              Distribute Equally
            </Button>
          </form>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
            This will distribute the total budget equally among all {categories.length} categories
          </p>
        </CardContent>
      </Card>
      ...
      {totalBudgetAmount > 0 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            <Card className="dark:bg-gray-800 dark:border-gray-700">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <TrendingUp
                  className={`h-4 w-4 ${remaining >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                />
              </CardHeader>
              <CardContent>
                <div
                  className={`text-xl md:text-2xl font-bold ${remaining >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
                  ${remaining.toFixed(2)}
                </div>
              </CardContent>
          ...
          <Card className="dark:bg-gray-800 dark:border-gray-700">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                ...
                {isOverBudget && <AlertTriangle className="h-5 w-5 text-red-600 dark:text-red-400" />}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-600 dark:text-gray-400">Budget: ${totalBudgetAmount.toFixed(2)}</span>
                </div>
                <Progress value={Math.min(spentPercentage, 100)} className="h-3" />
                <div className="text-center">
                  {isOverBudget ? (
                    <p className="text-red-600 dark:text-red-400 font-medium">
                      ⚠️ Over budget by ${(totalSpent - totalBudgetAmount).toFixed(2)} (
          ...
          <Card className="dark:bg-gray-800 dark:border-gray-700">
            <CardHeader>
              <CardTitle className="text-gray-900 dark:text-white">Category Budgets</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                {categoriesWithZeroBudget.length > 0 && (
                  <div className="border-t pt-4 dark:border-gray-600">
                    ...
                    >
                      {showZeroBudgets ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                      Show Other Categories ({categoriesWithZeroBudget.length})
                    </Button>

                    {showZeroBudgets && (
                      <div className="mt-4 space-y-4">{categoriesWithZeroBudget.map(renderCategoryBudget)}</div>
                    )}
                  </div>
                )}
              </div>
